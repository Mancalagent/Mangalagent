...
class MCTSTrainer:
    def __init__(self, **kwargs):
        ...
        self.tree = MCTSTree(init_state=State(board), policy_network=self.policy_network,
...
if __name__ == "__main__":
    trainer = MCTSTrainer(game_count=10)
    trajectories = trainer.train(progress_bar=True)

    # Print the first trajectory for verification
    if trajectories:
    ...
    Util.save_tree(file_path='mcts_tree.json')
